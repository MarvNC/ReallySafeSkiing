...
async function renderLogo(
  ...
  try {
    ...
    await page.setViewport({ width: 1600, height: 800, deviceScaleFactor: options.scale });

    // Navigate with cache busting to ensure fresh render
    const cacheBuster = Date.now();
    const renderUrl = `${serverUrl}/scripts/render-page.html?component=logo&t=${cacheBuster}`;
...
async function main(options: Options): Promise<void> {
  ...
  try {
    ...
    // Render the logo
    await renderLogo(options, browser, viteServer.url);
