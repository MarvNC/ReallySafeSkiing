...
export const GameLogo: FC<{ className?: string }> = ({ className }) => {
  return (
    <div className={clsx('flex flex-col items-center justify-center select-none p-4', className)}>
      {/* Top part: "REALLY SAFE" - stylized like a warning label tape */}
      <div className="relative z-10 -rotate-2 transform transition-transform duration-500 hover:rotate-0 hover:scale-105">
      ...
      <div className="relative -mt-2 transition-transform duration-500 hover:scale-105 md:-mt-4">
        ...
        <div className="absolute left-1/2 top-1/2 -z-10 h-full w-[120%] -translate-x-1/2 -translate-y-1/2 skew-x-[-20deg] bg-gradient-to-r from-transparent via-white/5 to-transparent blur-sm" />

        <h1 
          className="font-russo bg-gradient-to-b from-white via-sky-100 to-sky-400 bg-clip-text text-6xl tracking-tighter text-transparent uppercase italic drop-shadow-xl md:text-8xl lg:text-9xl"
          style={{ 
        ...
        {/* Low Poly Mountain Graphic from alternative logo */}
        <div className="absolute left-1/2 top-1/2 -z-20 -translate-x-1/2 -translate-y-1/2 scale-150 opacity-10">
        ...
        <div className="mt-2 flex justify-center gap-1 opacity-60">
           {[...Array(5)].map((_, i) => (
             <div key={i} className="w-8 h-1 bg-sky-300 skew-x-[-45deg]" />
           ))}
        </div>
      </div>
    </div>
